...
static inline int cmpxchg32(volatile uint32_t *ptr, uint32_t oldval, uint32_t newval)
{
#ifdef WIN32
    return _InterlockedCompareExchange32(ptr, newval, oldval) == oldval;
#else
    return __sync_bool_compare_and_swap(ptr, oldval, newval);
#endif
}

static inline int cmpxchg64(volatile uint64_t *ptr, uint64_t oldval, uint64_t newval)
{
#ifdef WIN32
