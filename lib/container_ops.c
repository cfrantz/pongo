...
static void search_helper(pgctx_t *ctx, dbtype_t *node, void *user)
{
    ...
    dbtype_t *value = _ptr(ctx, node->value);
    
    search->r = db_search(ctx, value,
            search->path, 
            search->n, 
            search->relop, 
            search->value, 
    ...
    if (search->r == 1 && search->n == 0)
        dbcollection_setitem(ctx, search->result, _ptr(ctx, node->key), value, 0);
}
...
int db_search(pgctx_t *ctx, dbtype_t *obj, dbtype_t *path, int n, relop_t relop, dbtype_t *value, dbtype_t *result)
{
    ...
    if (obj->type == Object) {
        if (n==0 || ((p->type == String || p->type == ByteBuffer) && !strcmp((char*)p->sval, "*"))) {
            ...
            for(i=0; i<o->len; i++) {
                ...
                if (r==1) {
                    ...
                    if (n==0)
                        dbcollection_setitem(ctx, result, _ptr(ctx, o->item[i].key), x, 0);
                    else
                        return 1;
                }
            }
        ...
        if (n==0 || ((p->type == String || p->type == ByteBuffer) && !strcmp((char*)p->sval, "*"))) {
            ...
            search.r = 0;
            bonsai_foreach(ctx, _ptr(ctx, obj->obj), search_helper, &search);
